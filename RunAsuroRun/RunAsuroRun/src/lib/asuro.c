...
/***************************************************************************
  \brief 
  Speicher für die ADC Werte aus IsrStandard().
...
/***************************************************************************
  ...
  Bemerkung: 
  Alle ADC Werte kommen durch die ADC hardware pipeline 2 Interrupts später an. 
  ...
  Somit wird erreicht, dass zumindest die Odometriemessung automatisch erfolgt. 
	...
	1) 
	In avg wird ein gleitender Mittelwert mitgeführt: a(n+1) = (a(n)+sensor)/4
	...
	passt sich avg sehr rapide den ADC-Werten an. Und verliert in der Realität 
	auch "nie" den ersten Tick. (So muss ASURO auch meist erst mal anfahren.) 

	3) 
	Allerdings - und hier habe ich noch nicht probiert - könnte es mit ein Teiler von 8 
